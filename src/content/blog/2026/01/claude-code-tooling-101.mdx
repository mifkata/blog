...
After setting up your [devcontainer](/blog/2025/weapons-of-choice-avoiding-malicious-ai-behaviour/)
the next obvious step is to configure **the reusable prompts** for your steady, production-ready
software development cycle using Claude Code. These reusable prompts, are the core of your
**Claude toolkit**, so maintain them well:

- **CLAUDE.md** - this is the primary instruction set that a **Claude agent instance**
  would load when started and after `/compact` completes. This is where you want to put as much nifty
  instructions that the bot works with your codebase. Having it and
  configured might seriously benefit _**token usage**_ and reduce agent _**planning time**_.
  Maitaining an efficient `CLUDE.md` can be considered an art form of its own.
- **Slash-commands** - a collection of reusable prompts that will help you trigger
  ...
  in your repository's `.claude/commands` directory.
...
<GithubLink url="https://github.com/mifkata/blog/tree/main/.claude/commands">
...
I'd restart my **Claude Code** to load the new `/commit-message`. Here's the actual
command from the blog's repository:

<GithubLink
  expanded
  url="https://github.com/mifkata/blog/blob/main/.claude/commands/commit-message.md"
/>
...
<GithubLink
  expanded
  url="https://github.com/mifkata/blog/blob/main/.claude/commands/spec-create.md"
/>

### /spec-update

I use this to keep the spec language compact and optimal, but sometimes I'd go
and update the spec manually, then call `/spec-apply` or `/spec-verify`.

<GithubLink
  expanded
  url="https://github.com/mifkata/blog/blob/main/.claude/commands/spec-update.md"
/>
...
<GithubLink
  expanded
  url="https://github.com/mifkata/blog/blob/main/.claude/commands/spec-refine.md"
/>

### /spec-apply

I call this after a spec is ready to implement. Sometimes when I have modified both
**specs** and **code**, I'd run `/spec-verify` instead.

<GithubLink
  expanded
  url="https://github.com/mifkata/blog/blob/main/.claude/commands/spec-apply.md"
/>
...
<GithubLink
  expanded
  url="https://github.com/mifkata/blog/blob/main/.claude/commands/spec-verify.md"
/>
...
<GithubLink
  expanded
  url="https://github.com/mifkata/blog/blob/main/.claude/commands/spec-refactor.md"
/>

### /spec-compact

This command is used by others such as `/spec-create`, but also is very nifty,
...
<GithubLink
  expanded
  url="https://github.com/mifkata/blog/blob/main/.claude/commands/spec-compact.md"
/>
...
different tech stack, being it a **web-based frontend application**, an **Elixir backend**,
**a service written in Rust**. We can safely assume that different tech stacks/apps, have
different testing instructions, as they would have different implementation specs.
...
<GithubLink
  expanded
  url="https://github.com/mifkata/blog/blob/main/.claude/commands/test-create.md"
/>
...
Analyses the current staged diff and generates a **Conventional commit message** with
meta information covering all staged changes.
...
Uses `/commit-message` to generate the message, asks for confirmation and commits. Requires
that `git config --global user.name [name]` and `git config --global user.email [name]` are
configured at least once in the container.

<GithubLink
  expanded
  url="https://github.com/mifkata/blog/blob/main/.claude/commands/commit-staged.md"
/>

## Commands: Generators

I don't recommend using AI to **scaffold code** for you, from my point of view, that's
a waste of tokens and computation time. However, there're cases where you want the agent
to generate code for you (and sometimes, we're just being lazy anyway).

### /post-create

An obvious example of a **super lazy code generator**. I intend to replace this with a
proper scaffolding command, but will keep it for historical purposes.

<GithubLink
  expanded
  url="https://github.com/mifkata/blog/blob/main/.claude/commands/post-create.md"
/>

### /tags-update

The landing pages for tags, such as the [ai tags page](https://mifkata.com/tags/ai/)
have custom title and description and I save them in [src/data/tags.json](https://github.com/mifkata/blog/blob/main/src/data/tags.json)
inside my repository. Because I'll add new tags and I'm either too lazy or forget to
update the list, I simply run `/tags-update` and **Claude** does the job for me.

You can imagine that with hundreds of entries this can be a very slow and token consuming
task, so consider this when creating and running pure content generators like this.

<GithubLink
  expanded
  url="https://github.com/mifkata/blog/blob/main/.claude/commands/tags-update.md"
/>

## Final thoughts

The more I work with **Claude Code**, the more projects I start. Keeping **chaos to
the minimum** is very key in long-term development, as agents are capable of producing
ridiculous amounts of code and the same time **technocal debt** and keeping your
...
However, if you do find a replacement toolkit, please let me know in the comments.
Happy coding!
