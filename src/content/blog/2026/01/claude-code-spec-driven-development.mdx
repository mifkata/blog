...
updatedDate: Feb 22 2026 21:30:00 GMT+0000 (UTC)
heroImage: "@/assets/blog/2026/0102-spec-driven-development.png"
tags: ["ai", "coding", "claude", "devex", "howto", "sdd"]
---
...
<Highlight block>
  > **Having migrated Claude Code tooling:** I generated couple of specs for existing components using <ClaudeCommand archived name="spec-create" />. After having them to my liking, I effectively created a pattern for future specs. Naturally, I asked **Claude Code** to generate missing specs for components, pages and layout following the pattern.

</Highlight>

<GithubLink url="https://github.com/mifkata/blog/tree/main/public/archive/specs">
  ...
  **fully generated and with minimal manual modifications**. Some of them are
  ...
  These since **end of February 2026** have been [migrated to openspec](https://github.com/mifkata/blog/tree/main/public/archive/openspec-specs).
</GithubLink>
...
While working with an agent, after a certain token limit is reached, it is going to automatically compact the context (conversation) that they're working with. In **Claude Code** this can be manually triggered with the <ClaudeCommand archived name="compact" external /> command.
...
I'm going to start, like always, with <ClaudeCommand archived name="spec-create" />
...
> I want to link slash-commands in my articles to either https://github.com/mifkata/blog/blob/main/public/archive/commands/<command-name>.md for internal commands and to https://code.claude.com/docs/en/slash-commands for external; there should be either a Github or Claude icon next to the link; link should be inline;

````
</WrapText>

The agent generated a long, explicit, human-readable spec, the opposite of my instructions to generate short, AI friendly specs. It's very weird to me that I keep compacting my specs, the agent keeps reading them, yet for some reason, especially with a fresh context, or when I've simply didn't work on the app for more than 8-10 hours, it generates more explicit and human-readable content and I have to manually run <ClaudeCommand archived name="spec-compact" /> to minify it.
...
- internal: `github.com/mifkata/blog/blob/main/public/archive/commands/{name}.md`
...
implementation using <ClaudeCommand archived name="spec-apply" />.
...
● Spec applied. Summary:
  ...
  <ClaudeCommand archived name="commit-message" />           <!-- GitHub icon → repo link -->
  <ClaudeCommand archived name="help" external />            <!-- Claude icon → docs link -->
````
...
even though that wasn't part of the spec. I run <ClaudeCommand archived name="spec-verify" /> to see how the
...
the changes to the repo. I `git add` the spec, component and stories and run <ClaudeCommand archived name="commit-staged" />.
...
- even if outdated, we can continue to work on top of it, by:
  ...
  > 2. Modify it manually, using <ClaudeCommand archived name="spec-update" /> or <ClaudeCommand archived name="spec-verify" />, or by having a conversation with the agent
  > 3. Ask for changes to be implemented

<Highlight block>
  > **NOTE:** this approach wouldn't be so valid for very large specs, which are affected by multiple smaller specs (components). Updating heavily updated larger specs can and should go through a process of heavier audit and verification. <ClaudeCommand archived name="spec-verify" /> is the command I rely on in such cases.
</Highlight>

I have manually added a **title** property in the code of `<ClaudeCommand />` and I want the spec to reflect it using <ClaudeCommand archived name="spec-verify" />. I ran the command without `[spec]` argument, the bot assumed `all` and went to verify all specs, so I had to cancel and rerun the command again.
...
Normally I'd go and search and manually implement the change, but come on, we have the right tooling to the get the job done. This time I'm using the <ClaudeCommand archived name="spec-refactor" /> command and hopefully after this change, this command annotation will become a link.
...
● Refactoring Plan: ClaudeCommand
  ...
  - <ClaudeCommand archived name="commit-staged" /> (internal - links to repo)
  - <ClaudeCommand archived name="compact" external /> (external - links to Claude docs)
...
To create test cases I use the <ClaudeCommand archived name="test-create" /> command, which
relies on `Testing.md` spec to be read before.

<GithubLink url="https://github.com/mifkata/blog/blob/main/public/archive/specs/Testing.md">
