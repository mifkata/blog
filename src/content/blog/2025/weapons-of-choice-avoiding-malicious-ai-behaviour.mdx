...
updatedDate: Dec 31 2025 09:30:00 GMT+0000 (UTC)
heroImage: "@/assets/blog/2025/1230-devcontainers.png"
tags: ["coding", "ai", "devcontainers", "claude"]
---

import RedditEmbed from "@/components/RedditEmbed/RedditEmbed.astro";
...
**NOTE:** In mine I also install `zsh` with [oh-my-zsh](https://ohmyz.sh/) for
my personal shell pleasure. I also set up `pnpm` and grant `sudo` privileges
to the `vscode` user. I do this for **debugging only**, you shouldn't do this for
your **prod containers**!
...
case "$CMD" in
  notify)
    terminal-notifier -title "Claude" \
      -message "$ARG" \
      -open "iterm2://session"
    ;;
  *)
...
- Run your applications from the `devcontainer` when possible. Forward relevant
  ...
  **NOTE**: This is required, especially for people coding on `arm` with containers running
