...
if [[ "$TOOL_NAME" == "mcp__provider_delegator__invoke_agent" ]]; then
  ...
  if [[ ! "$AGENT" =~ ^($CODEX_ALLOWED)$ ]]; then
    cat << EOF
{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"Agent '$AGENT' cannot be invoked via Codex. Implementation agents (*-supervisor) must use Task() with BEAD_ID for beads workflow."}}
EOF
    exit 0
  fi
fi
